...
pub fn measure_perf(device: &CLDevice) -> Result<std::time::Duration, Error> {
    ...
    let lhs = unsafe { create_buffer::<i32>(
        &device.ctx,
        MemFlags::MemReadWrite | MemFlags::MemCopyHostPtr,
        SIZE,
        Some(&vec![1i32; SIZE]),
    )? };
    let rhs = unsafe { create_buffer::<i32>(
        &device.ctx,
        MemFlags::MemReadWrite | MemFlags::MemCopyHostPtr,
        SIZE,
        Some(&vec![2i32; SIZE]),
    )? };
    let out = unsafe { create_buffer::<i32>(&device.ctx, MemFlags::MemReadWrite as u64, SIZE, None)? };
    ...
    for _ in 0..10 {
        ...
        unsafe { enqueue_nd_range_kernel(&device.queue, &kernel, 1, &[SIZE, 0, 0], None, None)? };
    }
